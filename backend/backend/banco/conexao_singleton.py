...
class Conexao:
    ...
    def __init__(
        self,
        host: str = "127.0.0.1",       # IP fixo para evitar erro de IPv6
        database: str = "BD_PESQUISADOR",   
        ...
        port: int = 5437,             # Porta do servidor de banco de dados
    ):
    ...
    def conectar_ao_banco(self):
        ...
        if self.__conexao is None:
            try:
                ...
                print("Conexão estabelecida com sucesso!")  # Mensagem de sucesso na conexão

            except OperationalError as erro:
                # Mensagem de erro se a conexão falhar
                print(f"Erro ao conectar ao banco: {erro}")
                # Resetamos a conexão para None caso queira tentar novamente
                self.__conexao = None
